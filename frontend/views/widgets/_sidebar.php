...
<aside class="sidebar">
    ...
    <div class="widget d_postlist">
        <div class="title"><h2><sapn class="title_span"><?=yii::t('frontend', 'Hot Recommends')?></sapn></h2></div>
        <ul>
            <?php
    ...
    <div class="widget d_tag">
        <div class="title"><h2><sapn class="title_span"><?=yii::t('frontend', 'Clound Tags')?></sapn></h2></div>
        <div class="d_tags">
            <?php
            $tags = Article::getTags(12);
            foreach($tags as $k => $v){
                echo    "<a title='' href='".Url::to(['search/index', 'q'=>$k])."' data-original-title='{$v}".yii::t('frontend', ' Topics')."'>{$k} ({$v})</a>";
            }
            ?>
        </div>
    </div>
    ...
    <div class="widget d_subscribe">
        <div class="title"><h2><sapn class="title_span"><?=yii::t('frontend', 'Email Suscribe')?></sapn></h2></div>
    ...
    <div class="widget d_comment">
        <div class="title">
            <h2><sapn class="title_span"><?=yii::t('frontend', 'Latest Comments')?></sapn></h2>
        </div>
        <ul>
            <?php
            ...
            <li>
                <a href="<?=Url::to(['article/view', 'id'=>$v['aid'], '#'=>'comment-'.$v['id']])?>" title="">
                    ...
                    <div class="muted"><i><?=$v['nickname']?></i>&nbsp;&nbsp;<?=yii::$app->formatter->asRelativeTime($v['created_at'])?> (<?=yii::$app->formatter->asTime($v['created_at'])?>)<?=yii::t('frontend', ' said')?>：<br><?=$v['content']?></div>
                </a>
            </li>
            <?php } ?>
        </ul>
    ...
    <div class="widget widget_text">
        <div class="title"><h2><sapn class="title_span"><?=yii::t('frontend', 'Frinendly Links')?></sapn></h2></div>
        <div class="textwidget">
            <div class="d_tags_1">
                <?php
