...
class Feehi extends Component{
    ...
    public function init()
    {
        ...
        if( ( $data=$cache->get($key) ) === false ){
            $data = Options::find()->where(['type'=>Options::TYPE_SYSTEM])->orwhere(['type'=>Options::TYPE_CUSTOM, 'autoload'=>1])->asArray()->indexBy("name")->all();
    ...
    private static function configInit()
    {
        ...
        if( !empty(yii::$app->feehi->smtp_host) && !empty(yii::$app->feehi->smtp_username) )  Yii::configure(yii::$app->mailer, [
    ...
    {
        Event::on(BaseActiveRecord::className(), BaseActiveRecord::EVENT_AFTER_INSERT, ['backend\components\AdminLog', 'create']);
        Event::on(BaseActiveRecord::className(), BaseActiveRecord::EVENT_AFTER_UPDATE, ['backend\components\AdminLog', 'update']);
        Event::on(BaseActiveRecord::className(), BaseActiveRecord::EVENT_AFTER_DELETE, ['backend\components\AdminLog', 'delete']);
